from API import APIKeys as keys, VkAPI as API
...
def calculations(fromId, friendsUserFrom, toId, friendsUserTo):
    print(type(friendsUserFrom))
    print(type(friendsUserTo))
    ...
    outputList = API.getExecute(API.getVkScriptForUsersExecute(allTheWays), keys.firstPair[1])
...
def makeFirstDivision(firstHalfOfFriends, secondHalfOfFriends):
    firstHalfFirstDivision = FirstDivision(keys.firstPair, firstHalfOfFriends, 'First half: ')
    secondHalfFirstDivision = FirstDivision(keys.secondPair, secondHalfOfFriends, 'Second half: ')
